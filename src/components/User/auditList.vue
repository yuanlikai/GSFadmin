<template>
  <Layout :style="{padding: '0 24px 24px'}">
    <Breadcrumb :style="{margin: '24px 0'}">
      <BreadcrumbItem>开发商 / 中介 未审核列表</BreadcrumbItem>
    </Breadcrumb>
  ...
  export default {
      ...
      return {
        ...
        columns1: [
          ...
          },
          {
            ...
            render: (h, params) => {
              let states;
              if (params.row.class_id === 2) {
                states = '中介'
              } else if (params.row.class_id === 1) {
                states = '开发商'
              }
              return h('div', [
                h('p', states)
              ...
              return h('div', [
                h('Poptip', {
                  props: {
                    confirm: true,
                    transfer: true,
                    title: '是否通过 开发商 / 中介 审核?',
                  },
                  on: {
                    'on-ok': () => {
                      let v = this;
                      v.Axios.post('/index.php/admin/user/shen', v.Qs.stringify({
    ...
    methods: {
      ...
      positList() {
        let v = this;
        v.loading2 = true;
        v.Axios.post('/index.php/admin/user/index3', v.Qs.stringify({
          pag: v.pag,
          num: 11
        })).then((res, req) => {
          v.loading2 = false;
