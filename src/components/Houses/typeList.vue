<template>
  <Layout :style="{padding: '0 24px 24px'}">
    <Breadcrumb :style="{margin: '24px 0'}">
      ...
      <BreadcrumbItem>类型列表</BreadcrumbItem>
    </Breadcrumb>
    ...
    <Modal
      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
        <FormItem label="类型名称" prop="name">
          <Input :maxlength="18" v-model="formValidate.name" placeholder="示例：普通住宅丶别墅丶商铺"></Input>
        </FormItem>
      </Form>
    </Modal>
  </Layout>
  ...
  export default {
                ...
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      ...
                      v.modal1 = true;
                      v.loading1 = true;
                      v.hou_type_id = params.row.hou_type_id;
                      v.formValidate.name = params.row.hou_type_title;
                    }
                  }
                }, '修改'),
                h('Poptip', {
                  ...
                  on: {
                    'on-ok': () => {
                      let v = this;
                      v.Axios.post('/index.php/admin/hou_type/del', v.Qs.stringify({
                        hou_type_id: params.row.hou_type_id
                      })).then((res, req) => {
                        if (res.data.error === 0) {
                          v.positList();
                        } else {
    ...
    methods: {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            if(v.state===0){
              data={
                hou_type_id:'',
                hou_type_title: v.formValidate.name,
              }
            }else {
              data={
                hou_type_id:v.hou_type_id,
                hou_type_title: v.formValidate.name,
              }
            }
            v.Axios.post('/index.php/admin/hou_type/add', v.Qs.stringify(data)).then((res, req) => {
              if (res.data.error === 0) {
                v.modal1 = false;
                v.positList();
      ...
      positList() {
        let v = this;
        v.loading2 = true;
        v.Axios.post('/index.php/admin/hou_type/index', v.Qs.stringify({
          pag: v.pag,
          num: 11
        })).then((res, req) => {
          v.loading2 = false;
